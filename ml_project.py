...
if app_mode == 'Prediction':
    ...
    model_choice = selectbox_without_default("Choose a model ⚙️", model_options)
    if not model_choice:
        st.stop()
    ...
    if st.button("Start training"):
        ...
        mlflow.sklearn.log_model(model, "top_model_v1")
        ...
        st.warning(f"🌱 Total emissions: {emissions_data['emissions'][0]} kg CO2eq")
...
if app_mode == 'Deployment 🚀':
    ...
    loaded_model
    number1 = st.number_input('alcohol',14.24)
    number2 = st.number_input('magnesium',100)
    number3 = st.number_input('color_intensity',1.1)
    number4 = st.number_input('proline',625)
    data_new = pd.DataFrame({'alcohol':[number1],
         'magnesium':[number2],
          'color_intensity':[number3], 'proline':[number4]})
...
st.markdown(f"####  Link to Project Website [here]({''}) 🚀 ")
...
def footer2():
    myargs = [
        "👨🏼‍💻 Made by ",
        link("https://github.com/gaetanbrison/summerschool", "Gaëtan Brison & Shreshtha Shaurya"),
        "🚀"
    ]
    layout(*myargs)
